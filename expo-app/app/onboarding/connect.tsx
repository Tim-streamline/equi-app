...
async function finishOnboarding() {
  try {
    await AsyncStorage.setItem('equinova:onboarded', '1');
  } catch {}
...
export default function ConnectScreen() {
  return (
    <View className="flex-1 bg-canvas">
      <SubHeader title="Verbind met Shelley" onBack={() => router.back()} />
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 180 }}>
        ...
        <View className="mb-4 rounded-2xl bg-teal-700 p-6">
          <View className="mb-4 h-14 w-14 items-center justify-center rounded-full bg-mint-500">
            <Text className="font-bold text-white" style={{ fontSize: 20 }}>S</Text>
          </View>
          <Text className="mb-2 font-bold text-white" style={{ fontSize: 22, lineHeight: 28 }}>
            Plan een gratis intake met Shelley.
          </Text>
        ...
        <View className="mb-3 rounded-2xl border border-ink-8 bg-white p-4">
          <View className="flex-row items-center gap-3">
            ...
            <View className="flex-1">
              ...
              <Text className="text-[13px] text-ink-50">Vrijdag 18 mei · 14:30</Text>
            </View>
          </View>
        </View>
      </ScrollView>
      <StickyCTA>
        <Button title="Plan intake & ga naar EquiNova" variant="deep" onPress={finishOnboarding} />
        <Button title="Sla over — ik kijk eerst rond" variant="ghost" onPress={finishOnboarding} />
      </StickyCTA>
    </View>
  );
}
