...
function ScreenConnect({ go, back }) {
  return (
    <div className="screen" data-screen-label="04 Connect Shelley">
      <SubHeader title="Verbind met Shelley" onBack={back} />
      <div className="body" style={{ padding: '0 20px 100px' }}>
        <div style={{ marginBottom: 20 }}>
          <div className="k" style={{ marginBottom: 6 }}>Stap 3 van 3 · Optioneel</div>
          <div className="progress-bar"><i style={{ width: '100%' }} /></div>
        </div>

        <div style={{ background: 'var(--app-deep)', color: 'white', borderRadius: 20, padding: 24, marginBottom: 18 }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--mint-500)', display: 'grid', placeItems: 'center', fontSize: 20, fontWeight: 700, marginBottom: 16 }}>S</div>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, lineHeight: 1.2, margin: 0, marginBottom: 8 }}>Plan een gratis intake met Shelley.</h2>
          <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: 14, lineHeight: 1.55, margin: 0 }}>
            30 minuten — we bekijken samen of mijn aanpak past bij jou en je paard. Zonder verplichting.
          </p>
        </div>

        <div style={{ background: 'white', border: '1px solid var(--app-border)', borderRadius: 16, padding: 16, marginBottom: 12 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--app-accent-soft)', color: 'var(--app-accent-strong)', display: 'grid', placeItems: 'center' }}>{I.calendar}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 14 }}>Eerstvolgende slot</div>
              <div style={{ color: 'var(--app-ink-3)', fontSize: 13 }}>Vrijdag 18 mei · 14:30</div>
            </div>
          </div>
        </div>
      </div>
      <div className="sticky-cta">
        <button className="btn-primary deep" onClick={() => go('home')}>Plan intake & ga naar EquiNova</button>
        <button className="btn-ghost" onClick={() => go('home')}>Sla over — ik kijk eerst rond</button>
      </div>
    </div>
  );
}
